(function () {
    ...
    const helpers = {
        ...
        loadChatRoom: evt => {
            ...
            chat.endUserName = evt.target.dataset.endUserName
        ...
        replyMessage: evt => {
            ...
            let createdAd = new Date().toLocaleString()
            ...
            $('#chat-msgs').prepend(
                `<tr>
                    <td>
                        <div class="sender">
                            ${chat.name} @ <span class="date">${createAt}</span>
                        </div>    
                    </td>
                </td>`
            )
        ...
        LoginIntoChatSession: function(evt) {
    ...
    chatBody.find('#loginScreenForm').on('submit', helpers.LoginIntoChatSession)
}());